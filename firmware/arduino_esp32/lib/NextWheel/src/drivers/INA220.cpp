...
uint8_t INA220::begin(
    uint8_t maxBusAmps,
    uint32_t microOhmR,
    const ina_Adc_Mode busAdcMode,
    const ina_Adc_Mode shuntAdcMode,
    const ina_Mode deviceMode,
    ...
    uint8_t numDevices)
{
    ...
    this->calibrationRegister = (uint64_t)409600000 / ((uint64_t)this->current_LSB * (uint64_t)microOhmR /
                                                       (uint64_t)100000);  // Compute calibration register
    ...
    if (maxShuntmV <= 40)
        ...
        programmableGain = 3;  // default gain x8 for +- 320mV
    ...
    this->configRegister |= (busAdcMode & 0xF)
                            << INA220_BADC_FIRST_BIT;  // Mask off unused bits then shift in the bus ADC mode
    this->configRegister |= (shuntAdcMode & 0xF)
                            << INA220_SADC_FIRST_BIT;  // Mask off unused bits then shift in the shunt ADC mode
    ...
    Wire.begin();
    uint8_t availableDevices =
        resetAll();  // Check if communication is working, then write calibration and configuration registers
...
void INA220::setMode(const uint8_t mode, const uint8_t deviceNumber)
{
    ...
    configRegister =
        readWord(INA_CONFIGURATION_REGISTER, getDeviceAddress(deviceNumber));  // Get the current register from device
...
uint16_t INA220::getBusRaw(const uint8_t deviceNumber)
{
    ...
    uint16_t raw =
        readWord(INA_BUS_VOLTAGE_REGISTER, getDeviceAddress(deviceNumber));  // Get the raw value from register
...
int16_t INA220::getShuntRaw(const uint8_t deviceNumber)
{
    ...
    int16_t raw =
        readWord(INA220_SHUNT_VOLTAGE_REGISTER, getDeviceAddress(deviceNumber));  // Get the raw value from register
    return (raw);
}
...
int32_t INA220::getBusMicroAmps(const uint8_t deviceNumber)
{
    ...
    int32_t microAmps =
        (int64_t)readWord(INA220_CURRENT_REGISTER, getDeviceAddress(deviceNumber)) * this->current_LSB / 1000;
    return (microAmps);
}
...
void INA220::triggerConversion(const uint8_t deviceNumber)
{
    /*! @brief     Starts a new conversion if the device is in triggered mode
        @details   Writes back the configuration register, triggering shunt, bus, or both conversions based on the
       device mode
        @param[in] deviceNumber to return the value for */

    int16_t configRegister =
        readWord(INA_CONFIGURATION_REGISTER, getDeviceAddress(deviceNumber));  // Get the current register
    writeWord(
        INA_CONFIGURATION_REGISTER,
        configRegister,
        getDeviceAddress(deviceNumber));  // Write back to trigger next conversion
}
...
uint8_t INA220::getDeviceAddress(const uint8_t deviceNumber)
{
    ...
    if (deviceNumber > this->numDevices)
        return 0;
...
uint8_t INA220::reset(const uint8_t deviceNumber)
{
    ...
    {  // If no I2C error
        writeWord(
            INA_CONFIGURATION_REGISTER,
            INA_RESET_DEVICE,
            getDeviceAddress(deviceNumber));  // Forces INAs to reset
        uint16_t tempRegister =
            readWord(INA_CONFIGURATION_REGISTER, getDeviceAddress(deviceNumber));  // Read the newly reset register
...
bool INA220::conversionFinished(const uint8_t deviceNumber)
{
    ...
    uint16_t cvBits =
        readWord(INA_BUS_VOLTAGE_REGISTER, getDeviceAddress(deviceNumber)) & 2;  // Bit 2 set denotes ready
...
bool INA220::waitForConversion(const uint16_t timeout, const uint8_t deviceNumber)
{
    ...
    uint16_t cvBits = 0;
    while (cvBits == 0)
    {  // Loop until the value is set or timeout is exceeded
...
void INA220::initDevice(const uint8_t deviceNumber)
{
    ...
    writeWord(
        INA_CALIBRATION_REGISTER,
        this->calibrationRegister,
        getDeviceAddress(deviceNumber));  // Write calibration value to device register
    writeWord(
        INA_CONFIGURATION_REGISTER,
        this->configRegister,
        getDeviceAddress(deviceNumber));  // Write new value to config register
}
